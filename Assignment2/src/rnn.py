...
class RNN(object):
	...
	def predict(self, x):
		...
		for t in range(len(x)):
			s[t] = sigmoid(self.V[:,x[t]] + np.dot(self.U, s[t-1]))
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			d_out = -y[t]
			d_out[d[t]] += 1
			d_in = np.dot(np.transpose(self.W), d_out)*grad(s[t])
			self.deltaW += np.outer(d_out, s[t])
			self.deltaV += np.outer(d_in, make_onehot(x[t], len(y[t])))
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			for t2 in reversed(range(max(t-steps,0), t)):
				d_in = np.dot(np.transpose(self.U), d_in)*grad(s[t2])
				self.deltaV += np.outer(d_in, make_onehot(x[t2], len(y[t2])))
				self.deltaU += np.outer(d_in, s[t2-1])
	...
	def generate_sequence(self, start, end, maxLength):
		...
		x = [start]
		t = 0
		s = np.zeros((maxLength+1, self.hidden_dims))
		y = np.zeros((maxLength, self.vocab_size))

		while t < maxLength and x!= end:
			s[t] = sigmoid(self.V[:,sequence[t]] + np.dot(self.U, s[t-1]))
			y[t] = softmax(np.dot(self.W, s[t]))
			y2 = y[t]
			x = multinomial_sample(y2)
			loss -= np.log(y2[x])
			sequence.append(x)
			t += 1
		return sequence, loss
...
if __name__ == "__main__":
		...
		for k in hidden_num:
			for i in learning:
				for j in look_up:
					r = RNN(vocab_size, k)
					if best_loss < 0:
						best_loss = r.train(X, D, X_dev, D_dev, epochs=10, learning_rate=i,anneal=5, back_steps=j, batch_size=100, min_change=0.0001, log=True)
						best_params = (k, j, i)
					else:
						xx = r.train(X, D, X_dev, D_dev, epochs=10, learning_rate=i, anneal=5, back_steps=j, batch_size=100, min_change=0.0001, log=True)
						if xx < best_loss:
							best_loss = xx
						best_params = (k, j, i)
	...
	if mode == "generate":
		...
		seq, loss = r.generate_sequence(start, end, maxLength)
		seq_words = []
		for i in range(0, len(seq)):
			seq_words.append(num_to_word[seq[i]])
		print(seq_words)
		print "Loss: %.03f" % loss
		print "Perplexity: %.03f" % np.exp(loss/(len(seq)-1))
